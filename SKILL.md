...
2. Search under `references/raw/mikuku-articles` using any available local file search method.
3. Prefer Markdown files that are clearly article bodies, published articles, or draft articles.
4. Open only the smallest relevant set of files needed to answer accurately.
5. Base the answer on the article text that was actually read.
6. If the local corpus is missing, empty, or does not contain relevant article body Markdown, say that local article context was unavailable.
